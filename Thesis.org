...
Analogues to the structural rules from logic exist for STLC. The only difference is that the type-theoretical variants provide additional information on how to handle terms. All three rules are listed in Figure [[fig:stlc_struct]].
...
#+caption: Structural rules for STLC
#+begin_figure
$$
\prftree[r]{Exchange}
...
The introduction and elimination rules for function types mirror the structure of implication deduction rules in intuitionistic logic.
...
$$
\begin{align*}
&\beta\text{-reduction: } ((\lambda x. t)\;s) = t[x := s]
\\[1ex]
&\eta\text{-conversion: } (\lambda x. (f\;x)) = f \text{ when $x$ is not free if $f$}
\end{align*}
#+end_figure
...
$$
\begin{align*}
&\beta\text{-reduction: } \fst{\tuple{s}{t}} = s
\\[1ex]
&\beta\text{-reduction: } \snd{\tuple{s}{t}} = t
\\[1ex]
&\eta\text{-conversion: } \tuple{\fst{s}}{\snd{s}} = s
\end{align*}
#+end_figure
