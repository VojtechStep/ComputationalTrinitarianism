...
\noindent for X ranging over atomic propositions. The rules of this fragment are given in Figure\nbsp[[fig:intuit_deduct]].
...
The $\tens$ ("tensor") product represents a pair of resources, both of which have to be consumed, due to the requirement of not discarding resources. The introduction rule looks almost exactly the same as the one for the $\with$ product, however in this case, the two parts of the tensor product are produced in different contexts. It is this difference that makes the two connectives have different semantics \mdash while the $\with$ product offers two different possible results from the same resources, the $\tens$ product combines two sets of resources into a pair of two results, and provides both for later consumption.
...
The reduction rule, shown in Figure [[fig:lin_tens_red]], describes how to perform such a completion. If the product is constructed using two proofs $t$ and $u$ of the judgments /$A$ avail/ and /$B$ avail/, respectively, then these proofs can replace the assumptions $\Lin{A\;avail}$ and $\Lin{B\;avail}$ in another proof $s$.

#+name: fig:lin_tens_red
#+caption: Tensor product proof reduction
#+begin_figure
$$
\prftree[r]{$\Elim{\tens}$}
 {\prfsummary[s]
