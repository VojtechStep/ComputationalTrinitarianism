...
There are other, equivalent, definitions of adjunct functors. We present one of them here, because the text to follow uses properties of the definition above as well as the one listed below. We leave out the proof that these two definitions are equivalent, since this work has been done already for example in \cite{MacLane1998}.

An adjunction is also determined by a functor $F: \cat{C} \to \cat{D}$ and for every object $D$ in $\cat{D}$, an object $G_0D$ in $\cat{C}$ with an arrow $\epsilon_D: FG_0D \to D$, such that for every arrow $f: FC \to D$ in $\cat{D}$, there is a unique arrow $f^{\flat}: C \to G_0D$, such that the following diagram commutes
#+begin_center
\begin{tikzcd}
...
Categories with all binary products come equipped with a *product functor* $\_ \times \_: \cat{C} \times \cat{C} \to \cat{C}$, which to each pair of objects $\tuple{C}{D}$ associates their product $C \times D$.
...
$$
This association is uniquely determined, because the identity arrows are unique, and so is the arrow $\prodar{\id_A}{\id_A}$.
